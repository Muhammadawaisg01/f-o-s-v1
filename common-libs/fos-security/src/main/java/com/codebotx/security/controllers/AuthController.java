...
import com.codebotx.security.repository.AuthRoleRepository;
...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        // AuthRole role = new AuthRole(AuthERole.VIEWER);
        // roles.add(role);

        if (strRoles == null) {
            AuthRole userRole = roleRepository.findByName(AuthERole.VIEWER)
                    .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
            roles.add(userRole);
        } else {
            strRoles.forEach(role -> {
                if (role.equalsIgnoreCase("content_creator")) {
                    AuthRole Role = roleRepository.findByName(AuthERole.CONTENT_CREATOR)
                            .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                    roles.add(Role);
                } else if(role.equalsIgnoreCase("upcoming_content_creator")){
                    AuthRole Role = roleRepository.findByName(AuthERole.UPCOMING_CONTENT_CREATOR)
                            .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                    roles.add(Role);
                } else if(role.equalsIgnoreCase("viewer")){
                    AuthRole Role = roleRepository.findByName(AuthERole.VIEWER)
                            .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                    roles.add(Role);
                } else {
                    AuthRole userRole = roleRepository.findByName(AuthERole.VIEWER)
